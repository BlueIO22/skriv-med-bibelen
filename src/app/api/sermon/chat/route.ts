...
const STEP_PROMPTS: Record<Step, string> = {
  tekststudie: `Du er en teologisk samtalepartner for en prest i Den norske kirke (DNK).
...
- Når du nevner bibelreferanser, bruk alltid standard norsk format: Bok kapittel:vers (f.eks. Joh 3:16, Rom 8:1-4, 1. Kor 15:3, 1. joh 1:1, 1. Johannes 1:1). Bøker med nummer skrives alltid med punktum etter nummeret: «1. Kor», «1. Pet», «1. Joh», «2. Sam», «1. Johannes» osv.

Du er en faglig ressurs, ikke en forfatter. Svar alltid på norsk.`,

  forbindelser: `Du er en teologisk samtalepartner for en prest i Den norske kirke (DNK).
...
- Når du nevner bibelreferanser, bruk alltid standard norsk format: Bok kapittel:vers (f.eks. Jes 53:5, Hebr 4:14-16)
...
- Når du nevner bibelreferanser, bruk alltid standard norsk format: Bok kapittel:vers
...
Du er en kritisk venn, ikke en ghostwriter. Svar alltid på norsk.`,
  ...
  samtale: `Du er en teologisk og homiletisk samtalepartner for en prest i Den norske kirke (DNK).
...
- Når du nevner bibelreferanser, bruk alltid standard norsk format: Bok kapittel:vers (f.eks. Joh 11:25, 1 Kor 15:3-4)
...
async function findRelatedVerses(query: string): Promise<string> {
  try {
    ...
    return `\n---\nRELEVANTE BIBELVERS FRA DATABASEN\nBruk disse aktivt og henvis til dem med full referanse (f.eks. Joh 3:16) i svaret.\n${lines}\n---`;
  } catch (e) {
...
---`;
  ...
  if (
    ...
    (step === "disposisjon" || step === "utkast")
  ) {
    prevNotes.push(`FORBINDELSESNOTATER:\n${ctx.forbindelserNotes}`);
  }
  if (ctx.disposisjonNotes?.trim() && step === "utkast") {
